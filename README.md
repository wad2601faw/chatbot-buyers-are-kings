...
├── api/
│   └── chat.php             (Main chatbot logic + product card generation)
├── assets/
│   ├── css/
│   │   └── style.css        (UI styling for cards & badges)
│   └── js/
│       └── chat.js          (Cart management & click handlers)
...
- **Badge animation**: Pulse zoom effect (0 → 1.15 → 1) in 0.5s
...
[Image] Nasi Kuning           [1]  ← Quantity badge
        dari Kedai Putra
        Rp 22000
        
[Image] Nasi Uduk             [2]  ← Quantity badge
        dari Toko Manis Bunda
        Rp 20000
```
- Cards are **clickable** → increases quantity
...
```
User: Click product 1 (×2), Click product 2 (×1)
